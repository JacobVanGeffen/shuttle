...
mod task_local {
    ...
    #[test]
    fn basic() {
        shuttle::task_local!(static TASK_LOCAL: RefCell<usize>);

        async fn do_test(n: usize) {
            TASK_LOCAL
                .scope(RefCell::new(1), async move {
                    TASK_LOCAL.with(|local| {
                        assert_eq!(*local.borrow(), 1);
                    });
                    TASK_LOCAL.with(|local| {
                        assert_eq!(*local.borrow(), 1);
        ...
        check_dfs(
            || {
                asynch::spawn(do_test(2));
                asynch::spawn(do_test(3));
                asynch::block_on(do_test(4));
            },
            None,
        );
    }
    ...
    fn nested_with() {
        shuttle::task_local! {
            static LOCAL1: RefCell<usize>;
            static LOCAL2: RefCell<usize>;
        }

        check_dfs(
            || {
                asynch::block_on(LOCAL1.scope(RefCell::new(1), async move {
                    LOCAL1.with(|local1| {
                        *local1.borrow_mut() = asynch::block_on(
                            LOCAL2.scope(RefCell::new(2), async move { LOCAL2.with(|local2| *local2.borrow()) }),
                        )
                    });
                    LOCAL1.with(|local1| assert_eq!(*local1.borrow(), 2));
                }));
                LOCAL1
                    .try_with(|_| {})
                    .expect_err("LOCAL1 should not be accessible outside of scope");
                LOCAL2
                    .try_with(|_| {})
                    .expect_err("LOCAL2 should not be accessible outside of scope");
            },
            None,
        );
    }
